# Spec — AstraNotes UI Refresh ("Refined Dark")

## Goal
Replace the current dark theme — which reads as "AI-generated" because of the ✦ sparkle
brand, the periwinkle gradient buttons (`--accent: #6c8cff`), low-contrast muted text, and
a flat, character-less type treatment — with a **crafted "refined dark"** look: a warmer,
higher-contrast palette with a single confident **amber** accent, a real typographic
hierarchy, flatter/quieter cards, and considered spacing and micro-interactions. Scope is
the **full treatment** (skin rewrite **plus** layout/component/hierarchy polish), using a
**polished system-font stack only** (no downloaded/bundled fonts). The redesign is purely
visual: every route, flow, and the 77 passing tests must behave exactly as before.
...
- **No light theme / theme toggle** this pass (could be added later via
  `prefers-color-scheme`; out of scope now).
...
- Surfaces: `bg #0D0F12` · `surface #16181D` · `surface-2 #1F222A` · `border #2A2E37`
- Text: `text #ECECEC` (≈AA on bg) · `muted #9BA1AC` (raised contrast vs. today's `#99a0b8`)
- Accent: `accent #E8A33D` (warm amber) · `accent-ink #15110A` (dark ink on amber for AA)
- Keep semantic flash colors (success/warn/danger) but retune for the new palette.
- Radius: tighten to `8px` cards / `6px` controls (less uniformly "soft"); add a subtle
  elevation/border system instead of flat sameness.
...
**Brand:** replace `✦ AstraNotes` with a crafted wordmark — a small inline-SVG monogram/mark
in the amber accent + the name in a tuned weight/tracking. No emoji, no sparkle.
...
## Open Questions
_(none — ready to implement on your go-ahead)_
